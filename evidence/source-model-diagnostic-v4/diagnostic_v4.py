...
def profiles(records,kernel):
 ...
 for delay in DELAY_GRID:
  ...
  for shift in base.SHIFTS:
   a=np.interp(base.TIME-shift,base.TIME,kernel,left=0,right=0);b=np.interp(base.TIME-shift-delay,base.TIME,kernel,left=0,right=0)
   coeff,res=bounded_gains(a,b,y);best=np.minimum(best,np.mean(res**2,axis=1))
  costs.append(best)
...
def fit(model,records,kernel,reference,v):
 ...
 def objective(p):
  ...
  return np.sqrt(np.maximum([np.interp(x,DELAY_GRID,row,left=z,right=z) if ok else z for x,row,z,ok in zip(d,table,null,valid)],0))
