...
def render(folder,family,seed):
 ...
 for j,source in enumerate(sources):
  ...
  for i,receiver in enumerate(receivers):
   ...
   alpha=1+rng.uniform(-1800,1800)*1e-6;offset=rng.uniform(.02,.10);times=np.arange(len(wave)+round(.12*rate))/rate;clean=np.interp((times-offset)/alpha,np.arange(len(wave))/rate,wave,left=0,right=0);noise=rng.normal(0,.00012,len(clean));y=.45*(clean+noise)
