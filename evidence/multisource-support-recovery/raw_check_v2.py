...
spec=importlib.util.spec_from_file_location('echosight.experimental_multisource',OUT/'experimental_multisource_v2.py');exp=importlib.util.module_from_spec(spec);spec.loader.exec_module(exp)
...
for family,seed in [('higher_order_four_sources',1129),('coordinate_frame_mismatch',1193)]:
 ...
 for name,r in outputs.items():
  ...
  (OUT/f'{family}-{seed}-{name}-raw-v2.json').write_text(json.dumps(r,indent=2)+'\n')
