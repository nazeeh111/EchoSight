...
def residual_profiles(parameters,model,records,kernel,reference,v):
 ...
 for shift in SHIFTS:
  ...
  if model=='single':
   ...
   b=np.interp((TIME[None,:]-shift-d[:,None]).ravel(),TIME,kernel,left=0,right=0).reshape(y.shape);b[~valid]=0.;bb=np.sum(b*b,axis=1);ab=b@a;by=np.sum(b*y,axis=1);det=aa*bb-ab**2;good=det>1e-9
