...
for kind,config,y,truth in rows:
 ...
 for name,m in methods.items():
  started=time.perf_counter();out=m.process_recording(y,fs,p,'development');predicted=np.array([c['delay_s'] for c in ([c for c in extract(out) if not c['unmodeled']] if name=='joint' else out['candidates'])]);trutharr=np.array(truth)
...
Path('work/physics-estimator/joint-guard-development.json').write_text(json.dumps(output,indent=2)+'\n')
for name in methods:
