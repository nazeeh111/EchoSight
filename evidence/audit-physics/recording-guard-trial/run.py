...
def cases():
    result=[]
    for suite,filename in [('eight','acceptance.json'),('twelve','acceptance_extended.json'),('stress','stress_acceptance.json')]:
        ...
        for case in spec.get('held_out_cases',spec.get('cases')):
            ...
            result.append((suite,case,spec,folder,'matched_filter'))
    # Same diffuse-null raw recordings, using the archived joint estimator in both arms.
    suite='stress';spec=json.loads((ROOT/'snapshot/evaluation/stress_acceptance.json').read_text());case=next(c for c in spec['cases'] if c['family']=='diffuse_null');folder=REPO/'work/physics-estimator/control-stress'/f"diffuse_null-{case['seed']}"
...
def main():
    ...
    for index,(suite,case,spec,folder,estimator) in enumerate(cases()):
        ...
        for method,func in [('original',ORIGINAL_INFER),('held_guard',guarded)]:
            ...
            truth=json.loads((folder/'truth.json').read_text());metrics=score_surfaces(result,truth,**{k:v for k,v in spec['matching'].items() if k!='one_to_one'})
            family=case.get('scenario',case.get('family'));name=f"{suite}-{family}-{case['seed']}-{estimator}-{method}";(ROOT/'results').mkdir(exist_ok=True)
            (ROOT/'results'/f'{name}.json').write_text(json.dumps(result,indent=2,allow_nan=False)+'\n')
        ...
        (ROOT/'results.json').write_text(json.dumps(dict(base_commit='198d365f3b13f221ac258332eaaccdf96506a641',rows=rows,source_unchanged=before==codehash(),source_sha256=before,same_raw_and_extracted_observations=True),indent=2,allow_nan=False)+'\n')
