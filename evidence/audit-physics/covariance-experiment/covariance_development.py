...
for family in ['white_integer','colored_fractional','clock_jitter_overlap']:
 for index in range(40):
  alpha=1. if family=='white_integer' else 1.0003;offset=.06 if family=='white_integer' else .06319
  first=.012 if family=='white_integer' else .012013;gap=.014 if family=='white_integer' else .00025
  ...
  if family=='colored_fractional':noise=lfilter([np.sqrt(1-.85**2)],[1,-.85],noise)
  y+=noise
...
Path('../physics-estimator/covariance-development.json').write_text(json.dumps(dict(classification='Monte Carlo development simulation, not physical coverage',seed=20261023,bootstrap_draws=31,repeats=7,elapsed_seconds=time.perf_counter()-started,rows=rows),indent=2)+'\n')
