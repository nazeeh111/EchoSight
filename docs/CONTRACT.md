...
EchoSight uses plain JSON at public boundaries and NumPy arrays inside numerical processing. The backend is a research prototype; software execution is not physical validation. Session, job and scene schemas use `schema_version: "1.0"`; new display comparisons use `1.1` with legacy1.0 track-carry support. See [HTTP endpoints and limits](API.md), [frontend handoff](FRONTEND_HANDOFF.md), and [schemas](../schemas/).
...
## Admission and cancellation boundaries

Ordinary raw mapping compares known native declarations of source configuration, emitted probe identity and playback route before spatial inference. Multiple different known values return `no_result` with `native_source_declarations_conflict`; original observations, waveform hashes and declarations remain available. The optional `source_declaration_consistency` summary distinguishes `contradictory`, `unknown`, `incomplete` and `consistent_declarations`. Missing/unknown legacy metadata does not manufacture a conflict or certify consistency. Matching declarations are operator assertions, not proof of one acoustic center, correct calibration or an unchanged physical source. Reference calibration also withholds a proposal on this exact conflict; a geometry-only `no_result` does not prevent an otherwise valid reference calibration. The experimental multi-source entry applies the same check within each source session, including prepared replay, while allowing distinct configurations between separately calibrated source sessions. Its `source_declaration_consistency_by_session` summaries preserve that scope.

All public inference methods forward cooperative cancellation, including the direct-plane-grid and first-echo baselines. A cancelled inference returns no surfaces, dimensions, hypotheses, derived geometry covariance or measurement guidance, even when cancellation arrives after fitting but before ambiguity checks. Raw observations and diagnostics can remain for recovery. Stored-job cancellation also retains its existing atomic publication protection; cancellation does not delete recordings. Bounded numerical operations are not instantly preempted.
