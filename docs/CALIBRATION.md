...
The mapper treats that fitted calibration as independent of the mapping recordings and receiver survey errors. Direct use of its covariance therefore requires independent mapping audio and survey errors. Reusing calibration training audio or its receiver surveys introduces correlations that the current mapper cannot represent; new capture IDs or different file hashes do not establish independence, and shared survey systematic errors can persist after resurveying. Replaying the calibration recordings remains useful for reproducing the fit, but is not independent mapping validation. Supporting correlated reuse requires the calibration-to-observation cross covariance, not larger marginal uncertainties. A retained-data [handoff probe](../evidence/calibration-transfer-applicability/REPORT.md) demonstrates this limitation without claiming a room-geometry error.
