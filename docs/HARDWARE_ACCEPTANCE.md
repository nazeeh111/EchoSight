...
**Known software limit before execution:** The new synthetic raw-clock investigation found an accepted143ppm rate bias with99.65µs maximum pilot residual and a false direct-null echo. Therefore the residual/rate bounds above are necessary engineering gates, not an independent measurement of clock accuracy. H3 must retain full pilot arrivals, residuals, rate uncertainty and raw responses for both spacings; passing one short train cannot establish that the selected correlation peaks are physically correct. The recording-only correction is still under investigation. The original H3 limits are unchanged, and no own-device run has passed them.
