...
## Completed calibration-to-room transfer

The authorized 56-recording study at frozen runtime 5d64234 returned six joint-calibration planes in each room, but only 19/24 unique held paths. Transfer and promotion failed. Independent review found that even perfect truth would yield 20/24 under the chosen exact-one ±200 µs rule. Original failures and criteria are retained. The new evaluation-only catalog preflight prevents silently accepting that design flaw in a future experiment; it does not modify the failed score or production extraction.

[All results, saved-output replay, optional raw-input download and exact commands](../evidence/calibration-room-transfer/) distinguish fresh raw processing from replay of saved arithmetic. Incomplete original RMS fields describe unique subsets, never complete 24-path accuracy. The eight-record prospective follow-up is specified but unexecuted. Hardware and material qualification remain separate.
